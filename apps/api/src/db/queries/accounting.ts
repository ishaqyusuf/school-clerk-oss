...
export async function createSchoolFee(ctx: TRPCContext, data: CreateSchoolFee) {
	const { db, profile } = ctx;
	const termId = data.termId || profile.termId!;
	...
	const classroomConnect = data.classroomDepartmentIds?.length
		? { connect: data.classroomDepartmentIds.map((id) => ({ id })) }
		: undefined;
	...
	if (!data.feeId) {
			...
			data: {
				...
				feeHistory: {
					create: {
						...
						current: true,
						walletId,
						classroomDepartments: classroomConnect,
					},
				},
			},
		});
		...
		await db.$transaction(async (tx) => {
			...
			const existingTermHistory = await tx.feeHistory.findFirst({
				where: {
					feeId: data.feeId,
					termId,
					deletedAt: null,
			...
			if (existingTermHistory) {
				await tx.feeHistory.update({
					where: { id: existingTermHistory.id },
					data: {
						amount: data.amount,
						walletId,

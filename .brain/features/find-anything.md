...
## Overview
SchoolClerk now has a Midday-style global command palette inside the dashboard shell. Users can open it from the header or with `Cmd/Ctrl + K` to jump to pages, quick actions, student records, classroom records, and staff records.
...
| `apps/api/src/trpc/routers/search.routes.ts` | Tenant-aware optimized student/classroom/staff search |
| `packages/db/src/schema/migrations/20260425164000_find_anything_search_indexes/migration.sql` | `pg_trgm` and search indexes |
| `packages/db/src/schema/migrations/20260617120000_find_anything_classroom_search_indexes/migration.sql` | Classroom search indexes |

## Search Design
- `Pages` and `Quick Actions` are resolved locally from the dashboard navigation registry for instant results.
- `Students`, `Classrooms`, and `Staff` are resolved through a dedicated tRPC search router.
- Record search only starts after 2 characters to reduce avoidable database load.
- Student results open the full student profile at `/students/:studentId`.
- Classroom results open the existing classroom overview sheet through `/academic/classes?viewClassroomId=:departmentId&classroomTab=students`.
- Local suggestions include richer page aliases and focused quick actions for enrollment, student import, classroom creation, subjects, reports, payments, and collections.
...
- Trigram GIN indexes back fuzzy matching on:
  ...
  - classroom name
  - classroom stream/department name
  - staff name
  - staff email
- Partial indexes only cover active rows where `deletedAt IS NULL`.

## Future Expansion
- Add finance records as an additional dedicated search source.

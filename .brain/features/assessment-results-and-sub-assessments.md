...
- Classroom result review surfaces report-sheet query failures as a visible error state with retry and classroom-overview recovery actions instead of leaving users on an indefinite loading spinner. On small screens, the Term and Classroom report filters render side by side in a two-column grid.

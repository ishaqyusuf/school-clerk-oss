...
- Assessment recording shows gender as a compact `M` / `F` badge beside the student name instead of using a separate Gender column, keeping score entry focused on marks. Classroom result review tables may still expose richer gender controls where roster maintenance is part of the workflow.

...
- Assessment recording and classroom result table headers show `(-)` for an uncapped assessment's missing obtainable value instead of spelling out an uncapped label in the compact score grid.
...
- Admin users can access the same authenticated assessment-recording page from the Academics module's Assessment navigation section.
...
- The assessment recording subject filter shows per-subject filling progress as completed student rows over the full classroom roster. A student counts as complete only when every assessment configured for that subject has a recorded score; subjects without assessments show zero completed. The redundant overall filtered-student count badge is not shown.
- Assessment recording uses the document as its only vertical scroll owner. The document scrollbar remains the single visible vertical scrollbar, search and filter controls scroll away with the page, and the opaque two-row table header alone remains sticky at the top of the viewport. The score table uses fixed layout and truncates long student names so the default single-subject view does not create mobile horizontal overflow.
...
- The subject assessment editor modal uses a flat tool-panel layout: no nested card shell, flat assessment rows, simple bordered stat/form sections, and a title that includes the current subject name when opened from assessment recording.
- The subject assessment editor `Record submission` action opens `/assessment-recording` with `termId`, `deptId`, and `deptSubjectId` prefilled so the score-entry table is filtered to that subject. The action is hidden when the editor is already opened inside assessment recording.

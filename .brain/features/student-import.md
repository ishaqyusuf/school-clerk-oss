...
- `Skip` is a dashboard-only review action for matched or attention rows. It is disabled for no-match rows so ready imports cannot be accidentally omitted. Skipped rows are omitted from the `executeStudentImport` payload and counted in the review summary, but their row-level skipped/status badge is hidden to keep the review list quiet.
...
- Review rows display parsed `name`, `surname`, `otherName`, and compact `M` / `F` gender controls in one line. Name part badges omit field labels, show `-` when `otherName` is empty, and auto-switch the name line to RTL when Arabic script is detected. Missing gender is indicated by red styling on the `M` / `F` control instead of a separate warning badge, and quiet match/ready states do not show separate badges.
...
- The review and execution panels show a classroom scope summary with total, checked, executable, and attention counts per classroom so multi-classroom batches can be scanned without opening every row. In review mode, multi-classroom scope badges become filters with an `All` badge first; the active badge uses the selected state and scopes the visible review rows plus batch execution to that classroom.
...
- `Cancel Import` is available before execution and returns the operator to the initial import screen without writing new student records. The durable local draft remains available so proceeding again can restore the staged review choices.
...
- The import modal stores a versioned local draft in browser localStorage, including setup values, pasted rows, review tab/phase, fallback classroom, active classroom filter, checked rows, row decisions, manual genders, per-row classrooms, name edits, and promoted match selections. Refreshing the browser restores these operator inputs and reruns verification against freshly fetched school data.
- Draft state is cleared only after a clean full-scope import result with no failed rows. Failed, cancelled, waiting-worker, partial, or classroom-filtered imports keep the draft so the user can refresh and continue; rows that already landed are ignored through the fresh verification/current-term matching path rather than through persisted client-only imported flags.

### Candidate Metadata

The match UI exposes each candidate as `{student name} - {classroom}`. Confidence percentages and current-term badges remain part of the matching model, but the row-level candidate UI hides them to keep operator review compact.

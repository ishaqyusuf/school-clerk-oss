...
- The DB command routers run Prisma from `packages/db` so `packages/db/prisma.config.ts` can load root envs, apply the database profile, expose the resolved `DATABASE_URL`, and target SchoolClerk's actual Prisma schema directory at `packages/db/src/schema` with migrations under `packages/db/src/schema/migrations`.

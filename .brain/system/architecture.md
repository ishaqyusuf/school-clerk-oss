...
- Reuse their proven thinking around module boundaries, file/folder structure, page composition, analytics instrumentation, widget organization, trackers, and observability-oriented UI patterns where they fit SchoolClerk.
...
- `packages/jobs` also owns `process-finance-payment-import-job`. The finance
  ...
  rows.
- PostgreSQL profile selection uses the shared `local-infra-kit`. Root tooling
  loads `.env` plus exactly one of `.env.local`, `.env.dev`, `.env.preview`, or
  `.env.production`; the selected profile owns `DATABASE_URL`. The toolkit
  derives transient Docker Compose settings only when that URL identifies the
  local target, so School Clerk can retain host port `55432` without imposing

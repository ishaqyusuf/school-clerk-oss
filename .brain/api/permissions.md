...
- Dashboard forgot-password requests resolve the reset callback from the matched user's tenant domain instead of the incoming root host. When the current tenant header or custom domain matches one of the email's users, that tenant is preferred; otherwise the first matched user's primary verified custom domain is used when available, falling back to the same dashboard email URL builder used by staff invite links. Reset emails render the direct tenant reset-page URL with the token and email query, not the Better Auth API reset URL, and use the school name as the sender display name when available.

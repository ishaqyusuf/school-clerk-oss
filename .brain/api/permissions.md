...
- Dashboard forgot-password requests resolve the reset callback from the matched user's tenant domain instead of the incoming root host. When the current tenant header matches one of the email's users, that tenant is preferred; otherwise the first matched user's primary verified custom domain is used when available, falling back to `dashboard.{tenant}.school-clerk.com` in production.

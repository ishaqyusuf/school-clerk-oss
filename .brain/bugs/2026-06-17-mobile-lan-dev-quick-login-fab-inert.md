...
The original quick-login FAB used a Radix dropdown trigger. A first fix replaced that with React state and native buttons, but that still depended on hydrated React click handlers. The more resilient pattern for LAN fallback was to make the menu open with browser-native HTML semantics and make quick-fill use plain links/query parameters. That reset-token/login-page helper has since been replaced by a Halaalvest-style dev-only account picker that validates tenant membership in the login server action and creates a Better Auth session without mutating passwords.

## Fix Implemented

The previous dev quick-login FAB used native `details`/`summary` to open the menu without relying on React state. On the login screen, user rows were plain links to the same login page with quick-fill query parameters, and the login form declared a server-action `POST` fallback so non-hydrated sessions submitted through the server instead of performing a native `GET` that exposed credentials in the URL. Current quick login keeps the server-action boundary, but uses a login-page account picker and a dev-only Better Auth endpoint instead of the standalone `/dev-quick-login` route.
...
- Keep login-screen quick-login behavior explicit: selecting a dev account should not cross tenant boundaries, and the user should explicitly submit the form.
...
- Keep dev account sign-in behavior isolated to clearly named dev-only auth endpoints and keep tenant validation in the dashboard login action.

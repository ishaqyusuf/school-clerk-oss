...
- Assessment recording setup states show the term/classroom filters plus a current-selection summary instead of a bare unavailable message. Admin users can open the selected classroom overview side sheet on the Subjects tab from this state to fix classroom subjects or assessments without leaving score entry.

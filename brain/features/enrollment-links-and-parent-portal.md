...
- `apps/school-site/src/lib/enrollment/actions.ts` validates parent email/phone/date-of-birth, selected-class document uploads, passport-photo image types, surfaces readable Vercel Blob configuration errors, and dispatches the submission-success email through Resend when configured.
...
- `apps/api/src/db/queries/enrollment-links.ts` revalidates class requirements during approval, stores admission payment handoff metadata, applies fee histories, and sends the successful-admission email through Resend when configured. Real Resend API delivery has been smoke-tested successfully.
